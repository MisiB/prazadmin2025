<div>
        ...
        <x-table :rows="$banks" :headers="$headers">
            ...
            @scope('actions', $row)
                <div class="flex items-center space-x-2">
                    <x-button icon="o-pencil" class="btn-sm btn-info btn-outline" wire:click="edit({{ $row->id }})"
                        spinner />
                    <x-button icon="o-trash" class="btn-sm btn-outline btn-error" wire:click="delete({{ $row->id }})"
                        confirm="Are you sure?" spinner />
    ...
    <x-modal wire:model="accountmodal" title="Bank Accounts" box-class="max-w-3xl">
        <x-button class="btn btn-sm btn-primary btn-outline" @click="$wire.addaccountmodal = true"
            label="Add Account" />
