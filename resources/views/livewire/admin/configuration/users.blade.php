<div>
    ...
    <x-card title="Users" class="mt-5 border-2 border-gray-200" separator>
        <x-slot:menu>
            <div class="flex items-center gap-4">
                <x-input icon="o-magnifying-glass" wire:model.live="search" placeholder="Search users..."
                    class="max-w-sm" />
    ...
    <x-modal wire:model="modal" title="{{ $id ? 'Edit User' : 'New User' }}" separator box-class="max-w-3xl">
        ...
        <x-form wire:submit="save">
                                    ...
                                    @scope('actions', $role)
                                        <div class="flex space-x-2">
                                            @if (in_array($role->id, $this->selectedroles))
                                                <x-button icon="o-trash" wire:click="deleterole({{ $role->id }})"
                                                    wire:confirm="Are you sure?" spinner
                                                    class="text-red-500 btn-outline btn-sm" />
                                            @else
                                                <x-button icon="o-plus" wire:click="addrole({{ $role->id }})" spinner
            ...
            <x-slot:actions>
                <x-button label="Delete User" wire:click="delete('{{ $id }}')" :disabled="!$id"
                    wire:confirm="Are you sure?" class="btn-error" spinner />
