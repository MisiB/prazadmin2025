<div>
    <x-card>
        <x-button label="Import Tenders" wire:click="openImportModal" responsive icon="o-arrow-up-tray"
        ...
        <x-table :headers="$headers" :rows="$tenders" pagination>
            ...
            @endscope
            @scope('cell_tender_type', $tender)
                <x-badge :value="$tender->tendertype?->name ?? 'N/A'" class="bg-blue-500 text-white" />
            @endscope
            @scope('cell_supplier_categories', $tender)
                @php
                    $categories = $tender->suppliercategories;
                    
                    // If it's a string, try to decode it as JSON
                    if (is_string($categories)) {
                        // Remove brackets if present (e.g., "[GM002,GT002]" -> "GM002,GT002")
                ...
                @if(count($categories) > 0)
                    <div class="flex flex-wrap gap-1">
                        @foreach($categories as $category)
                            @if(!empty($category))
                                <x-badge :value="$category" class="bg-gray-500 text-black text-xs" />
                            @endif
                        @endforeach
                    </div>
                @else
            ...
            @scope('actions', $tender)
                <div class="flex space-x-2">
                    <x-button icon="o-eye" wire:click="gettender({{ $tender->id }})"
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="flex space-x-2 justify-end">
                                        <x-button icon="o-pencil" wire:click="getfee({{ $tenderfee->id }})"
                                            class="btn-outline btn-sm" />
                                        <x-button icon="o-trash" wire:click="deletefee({{ $tenderfee->id }})"
                                            class="btn-error btn-sm"
                                            wire:confirm="Are you sure you want to delete this tender fee?" />
                                    </div>
                                </td>
                            </tr>
                        @empty
    ...
    <x-modal title="Edit tender" wire:model="edittendermodal">
        <x-form wire:submit.prevent="savetender">
            <div class="grid grid-cols-2 gap-2">
                ...
                <x-select placeholder="Status" wire:model="status" :options="$statuses" option-label="name"
                    option-value="id" />


                <x-select placeholder="Tender Type" wire:model="tender_type" :options="$tendertypes" option-label="name"
                    option-value="id" />
    ...
    <x-modal title="{{ $tenderfee_id ? 'Edit tender fee' : 'Add tender fee' }}" wire:model="addtenderfeemodal">
        <x-form wire:submit.prevent="savetenderfee">
            <div class="grid grid-cols-2 gap-2">
                <x-select placeholder="Inventory Item" wire:model="inventoryitem_id" :options="$inventoryitems"
                    option-label="name" option-value="id" />
                <x-select placeholder="Currency" wire:model="currency_id" :options="$currencies" option-label="name"
                    option-value="id" />
    ...
    <!-- Import Modal -->
    <x-modal wire:model="importmodal" title="Import Tenders" separator with-close-button class="justify-center">
        <x-form wire:submit.prevent="importTenders">
            <x-file wire:model="file" label="Attach Tender File" />
            ...
            <x-slot:actions>
                ...
                <x-button label="Upload" icon="o-check" class="btn-primary" type="submit"
                    spinner="importTenders" />
            </x-slot:actions>
        </x-form>
    </x-modal>
</div>
