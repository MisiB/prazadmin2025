...
class Workplans extends Component
{
    ...
    public function getmonthlist()
    {
        if ($this->targetmatrix_id) {
            ...
            // If we're editing an existing workplan, try to get targetmatrix from workplan
            if ($this->id) {
                $workplan = $this->individualworkplanrepo->getindividualworkplan($this->id);
                if ($workplan && $workplan->targetmatrix) {
                    $targetmatrix = $workplan->targetmatrix;
                }
            }

            // If not found, try to get it from the targets collection
            if (! $targetmatrix && $this->target_id && $this->targets && $this->targets->isNotEmpty()) {
                $target = $this->targets->where('id', $this->target_id)->first();
                if ($target && $target->targetmatrices) {
                    $targetmatrix = $target->targetmatrices->where('id', $this->targetmatrix_id)->first();
                }
            }

            // If still not found, load directly from database
            if (! $targetmatrix) {
                $targetmatrix = Targetmatrix::find($this->targetmatrix_id);
            }
    ...
    public function saveworkplan()
    {
        ...
        if ($this->id) {
            ...
            $response = $this->individualworkplanrepo->updateindividualworkplan($this->id, $data);
        } else {
            // Create new workplan
            $response = $this->individualworkplanrepo->createindividualworkplan($data);
        }
        ...
        $this->reset([
            'id', 'output', 'indicator', 'target', 'weightage',
    ...
    public function getworkplan($id)
    {
        ...
        if ($workplan) {
            $workplan->load('targetmatrix.target.indicator.departmentoutput.output.outcome.programme');
            $this->id = $workplan->id;
            ...
            // Set target_id if targetmatrix is loaded
            if ($workplan->targetmatrix) {
                $this->target_id = $workplan->targetmatrix->target_id;
            }
