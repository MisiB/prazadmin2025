...
class Strategydetail extends Component
{
    ...
    protected $strategyImportService;
    ...
    public function boot(
        ...
        iIndicatorInterface $indicatorrepository,
        iprogrammeInterface $programmerepository,
        ioutcomeInterface $outcomerepository,
        ioutputInterface $outputrepository,
        idepartmentInterface $departmentrepository,
        IStrategyImportService $strategyImportService
    ) {
    ...
    {
        return $this->strategyImportService->downloadTemplate($this->strategy->id);
    }

    public function importStrategy(): void
    {
        $this->validate([
            'importFile' => 'required|file|mimes:xlsx,xls|max:10240',
        ]);

        $response = $this->strategyImportService->importStrategy(
            $this->strategy->id,
