...
class _departmentRepository implements idepartmentInterface
{
    ...
    {
        return $this->department_user->with('user', 'supervisor')->where('user_id', $user_id)->first();
    }
    ...
    {
        return $this->department_user->whereIn('department_id', $departmentIds)->pluck('user_id');
    }

    public function getuseridsbydepartmentid($departmentId)
    {
        return $this->department_user->where('department_id', $departmentId)->pluck('user_id');
    }

    public function getcountbydepartmentids(array $departmentIds)
    {
        return $this->department_user->whereIn('department_id', $departmentIds)->count();
    }

    public function getdepartmentuserbyuserid($userId)
    {
        return $this->department_user->where('user_id', $userId)->first();
    }
}
