...
class _budgetRepository implements ibudgetInterface
{
    ...
    public function importbudgetitems(int $budget_id, array $items): array
    {
        $imported = 0;
        $errors = [];
        ...
        foreach ($items as $index => $item) {
            try {
                ...
                $item['created_by'] = \Illuminate\Support\Facades\Auth::user()->id;
                $item['budget_id'] = $budget_id;

                $this->budgetitem->create($item);
                $imported++;
            } catch (Exception $e) {
        ...
        if ($imported > 0 && empty($errors)) {
            return [
                'status' => 'success',
                'message' => "{$imported} budget items imported successfully",
                'imported' => $imported,
                'errors' => $errors,
            ];
            ...
            return [
                'status' => 'partial',
                'message' => "{$imported} items imported with ".count($errors).' errors',
                'imported' => $imported,
                'errors' => $errors,
            ];
        }
